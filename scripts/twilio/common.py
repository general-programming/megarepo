...
def get_twilio_credentials_from_vault():
    ...
    try:
        ...
        if not client.is_authenticated():
            log.warning(
                "Vault client is not authenticated, falling back to environment variables"
            )
            return None, None
        ...
        log.warning(
            f"Failed to retrieve Twilio credentials from Vault: {e}, falling back to environment variables"
        )
        return None, None
