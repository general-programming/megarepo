"""Upload a NixOS machine's LXC tarball to a Proxmox node and create/start
the container from it.

Build the image first:

    just -f nix/justfile build_lxc_image pipebomb

Then run this script with the resulting store path:

    python3 scripts/provision-lxc.py \
        --node sea1-hv-2 --vmid 119 --hostname pipebomb \
        --tarball /nix/store/.../tarball/nixos-image-lxc-....tar.xz \
        --rootfs-storage local-zfs --rootfs-size-gb 128 \
        --memory-mb 262144 \
        --vault-path app/proxmox/sea1-erin \
        --proxmox-host sea1-hv-0.generalprogramming.org
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--node", required=True, help="Proxmox node to create the container on"
    )
    parser.add_argument(
        "--vmid", type=int, required=True, help="VMID for the new container"
    )
    parser.add_argument(
        "--tarball", required=True, help="Path to the built LXC tarball"
    )
    parser.add_argument(
        "--skip-upload",
        action="store_true",
        help="The tarball is already staged in --storage's vztmpl dir; only its basename is used. "
        "Pushing ~500MB through the API times out over a slow link — scp it into "
        "/var/lib/vz/template/cache on the node and pass this instead.",
    )
    parser.add_argument(
        "--vault-path",
    ...
    parser.add_argument(
        "--hostname",
        required=True,
        help="Container hostname; matches the NixOS machine name",
    )
    parser.add_argument(
        "--storage", default="local", help="Storage for the vztmpl upload"
    )
    parser.add_argument(
        "--rootfs-storage",
        default="local-lvm",
        help="Storage for the container's rootfs",
    )
    parser.add_argument("--rootfs-size-gb", type=int, default=32)
    parser.add_argument("--bridge", default="vmbr0")
    parser.add_argument(
        "--cores", type=int, default=0, help="0 = all cores visible on the node"
    )
    parser.add_argument("--cpulimit", type=int, default=0, help="0 = unlimited")
    parser.add_argument(
        "--cpuunits", type=int, default=512, help="Scheduler weight; default is 1024"
    )
...
def main() -> None:
    ...
    vault = hvac.Client()
    secrets = vault.secrets.kv.v2.read_secret_version(path=args.vault_path)["data"][
        "data"
    ]
    ...
    if args.skip_upload:
        print(f"Using pre-staged {args.storage}:vztmpl/{tarball_name}")
    else:
    ...
    print(
        f"Creating CT {args.vmid} ({args.hostname}) on {args.node}: "
