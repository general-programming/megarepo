...
def get_transactions(item_id, access_token, redis, get_inserted=False):
    ...
    date_from = (datetime.datetime.now() - datetime.timedelta(weeks=4)).strftime(
        "%Y-%m-%d"
    )
...
def push_plaid_transactions(transactions, redis):
    ...
    for transaction in transactions:
        ...
        account_groups[account_id] += (
            f"[{transaction['date']}] {clean_name} - **${transaction['amount']}**\n"
        )
    ...
    for account_name, account_data in account_groups.items():
        account_fields.append({"name": account_name, "value": account_data.rstrip()})
...
def push_discord_embed(
    title, description=None, fields=None, footer_text=None, testing=False
):
    embed_to_push = {
        "username": "Bankwatch",
        # Ass bleach pink
    ...
    if footer_text:
        embed_to_push["footer"] = {"text": footer_text}
    ...
    r = requests.post(webhook_url, json={"embeds": [embed_to_push]})
