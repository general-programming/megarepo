...
| `0xC6` cmd `0x20` sub 7 / sub 8 | `CDW12 = 0x0720` / `0x0820` | **DO NOT SEND.** Unidentified 71808-byte region (`1122 × 64 = 0x11880`). Unlike subs 0–6 these do not point at an existing section — they spawn a producer coroutine (`0x7ffa972c` / `0x7ffa43c0`), and `0x7ffa43c0` conditionally zeroes a DRAM counter at `0x7ff879f8 + (idx<<4) + 0x1f0`. Both contain `op0 = 0` custom opcodes this repo's decoder does not resolve. No erase or program primitive is reachable from either — so **not destructive, but not certified read-only.** Reachable while latched (command byte `0x20`). See `sn200-c6-dispatch.md` §4.1. |
...
| `0xEC` `Admin_VUC_Enable` | `CDW10=0`, `CDW11=0`, `CDW12 = 0x0100` (enable) or `0x0000` (disable), `CDW13 = 0x564F4944`, `CDW14 = 0x57415252`, `CDW15 = 0x414E5459` (`"VOIDWARRANTY"` big-endian), no data buffer | **Resolved 2026-08-04 — `sn200-ec-and-allowlist.md` §1.** Overlay row 10, handler static `0x3002b6c4`. Writes **one byte** at `0x7ff8f1dd` and nothing else; there is no third encoding. **Inert on a latched drive** — the flag's only two readers are a gate branch whose two arms converge (the sole caller passes `a2 = 0`, so the vendor-band arm is dead code) and a Get-Log-Page block guarded by an in-progress sanitize. Cannot widen the allow-list, cannot reach EEPROM, cannot write the boot marker: after validation its entire host-controlled input is **one bit**. Any malformed `0xEC` **clears** the flag before returning Invalid Field. **☠ The encoding in `sn200-attack-surface.md` §4.8 is two dwords off** (it places the magic in CDW11–CDW13). No reason to send it; recorded because it was the last unresolved allow-listed opcode. |
...
| `0xC6`/`0x20` subs 0–6 | `VUC Get Drive Log`, all reads | Sub-dispatch `0x30030d14`. Subs 0–6 are positively identified reads of firmware-owned sections; the source address is recomputed from a descriptor on every call, so there is no cursor and no offset. **Subs 7–8 are NOT in this row** — see the do-not-send row above; the earlier claim that they "route into the same `.CDH`-magic reader" is wrong (`sn200-crash-dump-retrieval.md` §1.2.4). |
| ~~`0xC6`/`0x30`~~ | — | **☠ NOT SAFE — see "UNKNOWN — do not send" below.** The name `VUC Reset Drive Stats` was on the wrong command byte: it belongs to command byte **`0x22`** (StrIds 1602/1604, handler `0x3003002c`, subs 0–4), and `0x22` is *rejected* while latched. Command byte `0x30` is a different seven-sub action family, identified 2026-08-04 as SMART / drive-statistics collection (`sn200-c6-30-family.md`). |
...
| `0xFF` pfail-dump erase | `CDW12 = 0x0603` (cmd `0x03` sub `6`) | Erases EEPROM section `0x0a` (PFCL). **Synchronous, no second operation, sets no boot marker.** ☠ **Two corrections to what this row used to say.** (1) The handlers are *not* byte-for-byte identical: the forward arms are near-identical, but the **resume** handlers differ, and that is exactly where `0x0503` schedules the re-init and `0x0603` does not (`sn200-oam-dispatch.md` §4.2). (2) The "PFCL-only latch → `0x0603` recovers the data" branch is **WITHDRAWN as a procedure**: the boot that latches on PFCL falls into the `UNEXSTRT` stub writer and stamps **CLOG** on that same boot, so a drive you can probe is always both-armed (`sn200-section-arming.md` §4). On a both-armed drive `0x0603` is inert with respect to the latch (the boot predicate is an OR) and **destroys the PFail crash dump** — an evidence-destroying no-op, not a harmless first step. |
...
(`sn200-c6-30-family.md`) as the SMART / drive-statistics *collection* family
(sub 0's worker emits StrId 1952 `"SMART update failed from one of the
managers - did not save to DDR"`). Its only effector is the internal mailbox
`0x7ffbacf4`; everything it writes is DRAM counter state. **No erase, no
program, no EEPROM section API, no marker write, no re-init verb `0x25`, no
namespace call, no L2P touch** — PROVEN over the full call closure, and PROC8
contains zero references to `0x7ff8c7ec`, `0x7ff8d200` or `0x80000003/8/9`.
Subs 2, 4 and 5 return immediately when startup type == 6, so on a **latched**
drive only subs 0, 1, 3, 6 and the default arm execute. It stays on this list
because it is an unnecessary action family one nibble from every read we type,
and because ⚠ it does **not** require `CDW10 == 0` — the earlier claim to the
contrary was backwards (§6.1 of that document), so a mistyped `0x0320` probe
carrying `CDW10 = 2` is accepted without even a log line; and
**`0xE6`** (a diagnostic-dump read that calls into the overlay window at
`0x7ffbc18c` **without requesting an overlay load** — only 1 of the 30 overlays
has a function entry at that offset, so which function it calls depends on which
command the host sent previously; `sn200-ec-and-allowlist.md` §5.3).
Naming these is not an invitation — every one of them is a neighbour
...
| 3 | `0x0303` | `13` (SBL), verb `1` (section **write**) — PROVEN by execution | ☠ **SBL EEPROM — permanent brick** | CATASTROPHIC |
