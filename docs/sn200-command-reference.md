...
| `0xC6` body reads | `CDW10=<dwords>`, `CDW13=<offset dwords>`, `CDW12 = 0x0420` (crash) / `0x0620` (pfail) / `0x0220` (string table) / `0x0020` (drive log) | Chunked or single-shot read of the section body. PROVEN side-effect-free: the handler's complete call-target set contains neither erase primitive (`0x30030aa0` nor `0x30031d10`); reads are idempotent and resumable. |
