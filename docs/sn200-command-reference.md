...
| ~~`0xC6`/`0x30` "VUC Reset Drive Stats"~~ | — | **☠ MOVED TO "UNKNOWN — DO NOT SEND". The name was on the wrong command byte.** `VUC Reset Drive Stats` (StrIds 1602/1604) is command byte **`0x22`**, handler `0x3003002c`, subs 0–4 — and `0x22` is *rejected* while latched. Command byte `0x30` is a **different, unidentified** seven-sub action family (handler `0x3002fe38`, subs `0x0030`…`0x0630`) which **is** admitted by the gate. The "zero-length internal control/handshake" observation was correct and still holds — the firmware rejects `CDW10 != 0`… in fact it *requires* `CDW10 == 0` for `0x22` and `0x30` alone — so `0x30` cannot return host data. What it *does* is unknown, and at least two of its arms mutate state. See `sn200-c6-dispatch.md` §5. |
...
admitted by the post-crash gate, forced to `CDW10 == 0` so it cannot return
data, arms that spawn workers and touch the interrupt dispatch table, and no
positive identification of any of them (`sn200-c6-dispatch.md` §5); `0xEC` (dispatches to overlay handler `0x7ffbc24c`; semantics
