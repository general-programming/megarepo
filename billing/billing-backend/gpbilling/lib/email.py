...
def send_email(action, email_address):
    ...
    message = EmailMessage(
        ...
        body=render_template("email/%s_plain.html" % action, base_domain=os.environ["BASE_DOMAIN"], email_token=email_token),
        html=render_template("email/%s.html" % action, base_domain=os.environ["BASE_DOMAIN"], email_token=email_token),
    )
    mail.send(message)
