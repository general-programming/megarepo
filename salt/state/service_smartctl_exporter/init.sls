{# containerized: needs root + raw device access either way, and docker is
   ...
   hypervisors for now #}
{# upstream stopped publishing version tags after v0.14.0; :master is the only
   way to get current code. the pull is guarded by an `unless`, so each host
   keeps its first-pulled image until the image is removed deliberately #}
{% set image = 'ghcr.io/prometheus-community/smartctl-exporter:master' %}
