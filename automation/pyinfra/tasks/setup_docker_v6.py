...
for dev, dev_config in host.get_fact(NetworkDevices).items():
    if dev_config["ipv6"]:
        ...
        dev_v6_addr = ipaddress.IPv6Network(
            f'{dev_config["ipv6"]["address"]}/{dev_config["ipv6"]["mask_bits"]}',
            strict=False,
        )
...
if v6_addr:
    ...
    docker_net = ipaddress.IPv6Network(
        v6_addr.network_address + docker_net_bits
    ).supernet(128 - 96)
